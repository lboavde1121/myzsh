...
COLOR_END="%{[0m%}"
# PROMPT="${COLOR_FG}%n@%m${COLOR_END} %{${fg[red]}%}[%~]% ${reset_color}%} %{${fg[blue]}%}%1v${reset_color}%}
# %{${fg[green]}%}$ %{${reset_color}%}"
PROMPT="%{${fg[yellow]}%}%n@%m %{${reset_color}%}%{${fg[red]}%}[%~]% ${reset_color}%} %{${fg[blue]}%}%1v${reset_color}%}
%{${fg[green]}%}$ %{${reset_color}%}"
...
zstyle ':completion:*' completer _complete _ignored _oldlist
...
# その他
alias garii="cd $GOPATH/src/github.com/Arii-Inc"
alias gg="go get -u "

alias free="top -l 1 | grep PhysMem"
alias discfree="diskutil info / | grep Free"
##################
function mcd() {
echo $1;
if [ $? -eq 0 ]
